...
const privacyPoliciesSlice = createSlice({
  ...
  extraReducers: builder => {
    builder.addCase(modules_privacy_policy_list.pending, (state, action) => {
    ...
    }).addCase(modules_privacy_policy_partial_update.pending, (state, action) => {
      if (state.api.loading === "idle") {
        state.api.loading = "pending";
      }
    }).addCase(modules_privacy_policy_partial_update.fulfilled, (state, action) => {
      if (state.api.loading === "pending") {
        state.entities = state.entities.map(record => record.id === action.payload.id ? action.payload : record);
        state.api.loading = "idle";
      }
    }).addCase(modules_privacy_policy_partial_update.rejected, (state, action) => {
      if (state.api.loading === "pending") {
        state.api.error = action.error;
        state.api.loading = "idle";
      }
