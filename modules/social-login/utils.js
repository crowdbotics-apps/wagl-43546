...
export const validateEmail = new RegExp("^[^\\s]+([.-]?\\w+)*@\\w+([.-]?\\w+)*(.\\w{2,3})+$"); // Change here the  name of the screen on navigator you want to redirect after Login
...
export const mapErrorMessage = error => {
  ...
  return errorStatements[message] ? { ...errorStatements[message]
  } : {
    code: null,
    message
  };
